            ...
            <div class="col-12">
                <div class="page-title-box">
                    <div class="page-title-right">
                        <a href="{{ route('question.create') }}" class="btn btn-primary rounded-pill">Create</a>
                    </div>
                            ...
                            <tbody>
                                @foreach ($show_data as $key => $value)
                                    <tr>
                                        ...
                                        <td>{{ $value->name }}</td>
                                        <td>{{ $value->answer }}</td>
                                        <td>{{ Str::limit($value->comment, 50) }}</td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="button-list">
                                                @if ($value->status == 'active')
                                                    <form method="post" action="{{ route('question.inactive') }}"
                                                        class="d-inline">
                                                        @csrf
                                                ...
                                                @else
                                                    <form method="post" action="{{ route('question.active') }}"
                                                        class="d-inline">
                                                        @csrf
                                                        <input type="hidden" value="{{ $value->id }}" name="hidden_id">
                                                        <button type="button"
