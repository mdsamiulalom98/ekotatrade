            ...
            <div class="col-12">
                <div class="page-title-box">
                    <div class="page-title-right">
                        <a href="{{ route('couponcodes.create') }}" class="btn btn-primary rounded-pill">Create</a>
                    </div>
                        ...
                        <table id="datatable-buttons" class="table table-striped dt-responsive nowrap w-100">
                            ...
                            <tbody>
                                @foreach ($data as $key => $value)
                                    <tr>
                                        <td>
                                            <span class="btn btn-default">{{ $loop->iteration }}</span>
                                        </td>
                                        <td>{{ Str::limit($value->product->name ?? 'All Product', 20) }}</td>
                                        <td>{{ $value->coupon_code }}</td>
                                        <td>{{ $value->expiry_date }}</td>
                                        <td>{{ $value->offer_type == 2 ? 'Amount' : 'Percentage' }}</td>
                                        <td>{{ $value->amount }}</td>
                                        ...
                                        <td>
                                            <div class="button-list">
                                                @if ($value->status == 1)
                                                    <form method="post" action="{{ route('couponcodes.inactive') }}"
                                                        class="d-inline">
                                                        @csrf
                                                ...
                                                @else
                                                    <form method="post" action="{{ route('couponcodes.active') }}"
                                                        class="d-inline">
                                                        @csrf
                                                        <input type="hidden" value="{{ $value->id }}" name="hidden_id">
                                                        <button type="button"
                                                ...
                                                <a href="{{ route('couponcodes.edit', $value->id) }}"
                                                    class="btn btn-xs btn-primary waves-effect waves-light"><i
                                                        class="fe-edit-1"></i></a>

                                                <form method="post" action="{{ route('couponcodes.destroy') }}"
                                                    class="d-inline">
                                                    @csrf
                                                    <input type="hidden" value="{{ $value->id }}" name="hidden_id">
                                                    <button type="submit"
